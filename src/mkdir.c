...
int create_dir(char *path, int recursive,mode_t perms, int verbose) {
    path[strcspn(path, "\r\n")] = 0;
    if (path[0] == '\0') {
        fprintf(stderr, "mkdir: path is NULL\n");
        return 1;
    }
    if (!recursive) {
        if (mkdir_with_mode(path, perms)!=0) {
            return 1;
        }
        if (verbose) {
            printf("%s\n",path);
    ...
    if (!dir_exists(dir_path)) {
        if (mkdir_with_mode(dir_path, perms)!=0) {
            return 1;
        }
        if (verbose) {
            printf("%s\n",dir_path);
...
int parse_mkdir_flags(int argc, char **argv, int *start) {
     ...
     while (i < argc && argv[i][0] == '-'  && argv[i][1] == '\0') {
         for (int j =1; argv[i][j] != '\0'; j++) {
             ...
             if (c == 'm') flags |= MKDIR_M;
             if ( c == 'v') flags |= MKDIR_V;
             else {
                 fprintf(stderr, "mkdir: unknown flag -%c\n", c);
                 return  -1;
             }
         }
         i++;
